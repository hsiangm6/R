...
#ctrl + L 清除console
#<- (賦值)(快捷鍵:alt + -)
#Run the current line or selection(快捷鍵: ctrl+enter)
x <- 1
print(x) #(=直接打x)
y <- "hello"
print(y)
z <- c(1:3) #向量(等於c(1,2,3))
print(z)
rm(x)#把x刪掉
...
class(2)
class(2L) #加L會變成整數類別
class(2.5L)#小數後加L會出警告
aa <- 2.5L
class(aa)

class(TRUE) #(= class(T)) #邏輯值類型
class(FALSE) #(= class(F))
class(True) #錯誤
8<7
...
#轉換失敗時會產生NA值(遺失值)
date_of_origin <- as.Date("1970-01-01") #文字類型轉換為日期類型
as.integer(date_of_origin) #日期轉換為整數
as.integer(date_of_origin + 1)
as.integer(date_of_origin - 1)

date_of_origin + 1
date_of_origin - 1
date_char + 1 #文字類型無法加減乘除
...
time_of_origin <- as.POSIXct("1970-01-01 00:00:00", tz = "GMT") #數字轉換為日期，tz=GMT代表時區
...
#R語言自動把答案轉換成了適當型別(R語言的彈性)
class(2+2) #數值類型的運算
class(2L+2L) #整數類型
class(2+2L) #數值類型
class(2L+8+TRUE)#數值
2L+8+TRUE #11
#在R語言中，TRUE跟1或者1L是相等的；FALSE跟0或者0L是相等的。

#自訂function
add <- function(x, y) { #x, y是要輸入的數字
  return(x + y)
}
add(5,8)

myMean <- function(x){ #建立myMean function
  m <- sum(x)/length(x)
  return(m)
}
myMean(1:10)
...
#變數型別的判斷(數值/整數/文字/邏輯值都是is.xxx)
#判斷是否為數值
is.numeric(8.7)
is.numeric("10")

#判斷是否為整數
is.integer(7) #7是數值類型，非整數
is.integer(80L)
...
gender <- factor(gender)   #轉換成factor型態，多了一個層級表示
gender #Levels(層級)的屬性代表:在這個變數裡面，存在哪些類別
levels(gender)

gender <- factor(gender, ordered = TRUE, levels = c("boy", "girl")) #小 -> 大
gender

gender <- factor(gender, ordered = TRUE) #level沒有設定大小，預設使用字母順序排序
gender
...
ID <- c(1, 2)
great_nba_teams <- cbind(great_nba_teams, ID) #新增一個欄位，執行一次就好
great_nba_teams["total_games"] <- great_nba_teams$wins + great_nba_teams$losses #另一種新增欄位的方式
great_nba_teams
nrow(great_nba_teams) #共有幾列(筆)

str(great_nba_teams)
great_nba_teams[ ,1] # 等於great_nba_teams[ ,"team_name"] 也等於great_nba_teams$team_name
...
#"math_score": 80,75,60,90,85 "reading_score": 75,85,90,65,70 "listening_score": 40,70,85,70,90
#新增一欄:"total_score"將每位學生的三科成績做加總，
#新增一欄:"average_score"將每位學生的三科成績做平均
student <- c("Bob", "Nick", "Mary", "Harry", "John")
studentID <- c(1,5,9,13,8)
math_score <- c(80,75,60,90,85)
reading_score <- c(75,85,90,65,70)
listening_score <- c(40,70,85,70,90)
Student_Info <- data.frame(student, studentID, math_score, reading_score,listening_score)
Student_Info["total_score"] <-Student_Info$math_score+Student_Info$reading_score+Student_Info$listening_score 
Student_Info["average_score"] <- (Student_Info$total_score)/3
Student_Info

#載入gapminder 資料
library(gapminder)
#install.packages("gapminder") 如果沒有套件，輸入此行就可下載
...
f1 <- gapminder %>%
  filter(year == 2007) #篩出2007資料

s1 <- gapminder %>%
  filter(year == 2007) %>%
  select(country) #只印出2007年的country

#新增衍生變數(新增欄位)(mutate)
gapminder_new <- gapminder %>%
  mutate(pop_in_thousands = pop / 1000) #(新增的欄位名稱= 塞進欄位的資料)
...
#重新命名欄位名稱(rename)
rename1<-rename(gapminder,Cont=continent) #continent改名為Cont
rename1

#練習
#有幾筆資料是continent為Asia的?
#法一
sum_continent <- gapminder %>%
  filter(continent=="Asia") 
nrow(sum_continent)
#法二
nrow(gapminder[gapminder$continent=="Asia", ])
#法三
gapminder %>% filter(continent=="Asia") %>% summarise(count=n()) #count=類別名稱, n()=?
...
#每個人的GDP和預期壽命之間的關係
scatter_plot <- ggplot(f1, aes(x = gdpPercap, y = lifeExp)) + #f1=2007年的資料
  geom_point() + #圖層用散佈圖
  geom_smooth(method = "lm") #lm=線性回歸
scatter_plot #正相關
...
gdpPercap_2007_na <- gapminder %>%
  filter(year == 2007 & country %in% c("China", "Japan", "Taiwan", "Korea, Rep.")) #篩出2007年這四個國家的資料
bar_plot <- ggplot(gdpPercap_2007_na, aes(x = country, y = gdpPercap)) +
  geom_bar(stat = "identity")
bar_plot
#stat預設值為”stat_count”，即每個x資料點對應的資料次數。 
#如果希望bar圖的高度呈現資料值(y的資料值)，則須將stat = “identity”。
...
mean(mtcars$mpg)
sd(mtcars$mpg)
quantile(mtcars$mpg, probs=0.75)  # 第三分位數
